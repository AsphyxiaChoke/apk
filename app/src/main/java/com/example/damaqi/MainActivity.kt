...
fun AuditTab() {
    ...
    Column(
        ...
        LabeledField("允许次数", allow) { allow = it }
        ...
        Button(
            onClick = {
                ...
                val lm = parseLong(last, true) ?: return@Button toast(ctx, "前期无效")
                val cm = parseLong(curr, true) ?: return@Button toast(ctx, "当期无效")
                val a = parseLong(allow) ?: return@Button toast(ctx, "次数无效")
        ...
        Text("金皇冠：清零^(91^)、重置^(92^)、爆机^(93^)、永久^(99^)", color = Color.Gray, fontSize = 12.sp)
        Text("西游：清零^(91^)、重置^(92^)、爆机^(93^)、极难^(94^)、超难^(95^)、永久^(99^)", color = Color.Gray, fontSize = 12.sp)
    }
}
...
fun ConfigTab() {
    ...
    Column(
        ...
        Text("注：参数与机芯打码相同", color = Color.Gray, fontSize = 12.sp, modifier = Modifier.padding(bottom = 8.dp))
        LabeledField("线号", line) { line = it }
        LabeledField("机号", machine) { machine = it }
        LabeledField("序列号", serial) { serial = it }
